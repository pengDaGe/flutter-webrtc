...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    result(nil);
        ...
        [self initLoggerCallback:severity];
            ...
            // Fallback: if audioTrack is nil, try to pick any local audio track
            if (audioTrack == nil) {
                for (NSString* key in self.localTracks) {
                    id<LocalTrack> local = self.localTracks[key];
                    if (local != nil && [local isKindOfClass:[LocalAudioTrack class]]) {
                        LocalAudioTrack* lat = (LocalAudioTrack*)local;
                        audioTrack = lat.audioTrack;
                        break;
                    }
                }
            }
            ...
            if (track != nil && [track isKindOfClass:[RTCVideoTrack class]]) {
                ...
                if (audioTrack != nil) {
                    [recorder attachAudioTrack:(RTCAudioTrack*)audioTrack];
                } else {
                    // Retry to attach local audio track shortly after start, in case GUM audio arrives late
                    __weak typeof(self) weakSelf = self;
                    __block int attempts = 40; // ~2s at 50ms interval
                    void (^tryAttach)(void) = ^{
                        if (attempts-- <= 0) {
                            return;
                        }
                        ...
                        RTCAudioTrack* fallback = nil;
                        for (NSString* key in strongSelf.localTracks) {
                            id<LocalTrack> local = strongSelf.localTracks[key];
                            if (local != nil && [local isKindOfClass:[LocalAudioTrack class]]) {
                                fallback = ((LocalAudioTrack*)local).audioTrack;
                                break;
                            }
                        }
                        FlutterRTCMediaRecorder* rec = strongSelf.recorders[recorderId];
                        if (fallback != nil && rec != nil) {
                            [rec attachAudioTrack:fallback];
                        } else {
